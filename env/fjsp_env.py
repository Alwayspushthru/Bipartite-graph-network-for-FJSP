...
class FJSPEnv:
    ...
    def set_initial_data(self, job_length_list, op_pt_list):
        ...
        self.number_of_ops = self.op_pt.shape[1]  # 每个文件的总工序数,同工序的话就是 (工件数 x 工序数/机器数)
        ...
        # Masks and relations
        self.process_relation = self.op_pt != 0     # [B, O, M]
        self.reverse_process_relation = ~self.process_relation

        self.compatible_op = np.sum(self.process_relation, axis=2)
        self.compatible_mch = np.sum(self.process_relation, axis=1)
    ...
    def step(self,actions):
        ...
        candidate_add_flag = (chosen_op != self.job_last_op_id[self.env_idxs, chosen_job]) # 候选动作推进flag，只要选择的动作不是工件最后一个操作
        self.candidate[self.env_idxs, chosen_job] += candidate_add_flag # 那么对应的工件就+1
        self.mask[self.env_idxs, chosen_job] = (1 - candidate_add_flag) # 标记对应环境的对应工件是否已完工

        pt = self.unmasked_op_pt[self.env_idxs, chosen_op, chosen_mch]
        ...
        self.candidate_process_relation[mask_temp, chosen_job[mask_temp]] = \
            self.reverse_process_relation[mask_temp, chosen_op[mask_temp] + 1] # 更新可行性关系
        self.candidate_process_relation[~mask_temp, chosen_job[~mask_temp]] = 1 # 对于已经完成加工的工件，全部置为1
        ...
        self.op_ct[self.env_idxs, chosen_op] = chosen_op_st + self.op_pt[self.env_idxs, chosen_op, chosen_mch]
        idle_inc = np.maximum(0.0, chosen_op_st - self.mch_free_time[self.env_idxs, chosen_mch])
        self.idle_acc[self.env_idxs, chosen_mch] += idle_inc
        ...
        self.done = np.ones(self.number_of_envs) * (self.step_count >= self.number_of_ops)
